...
const projects: { name: string; description: string; links: ProjectLink[] }[] = [
  {
    ...
    description:
      "A live marine forecast for my home water, Charlotte Harbor and Pine Island Sound. Parses the NWS coastal waters product into wind, chop, and storm-risk cards, then adds Port Boca Grande tides with a tide-plus-light fishing read, live station observations, and NEXRAD radar cropped to the harbor. A verdict line reads the day's chop, storm risk, and rain into a plain call on whether to go. It shows official NWS marine warnings and advisories the moment they are issued, and as an early backstop the browser samples the radar over the harbor and the pass every few minutes, flagging a cell on the water that the inland airport observation would miss. GitHub Actions polls after each NWS issuance and republishes the page as static HTML; the browser refreshes the live pieces on every load. Runs entirely on public NOAA endpoints and GitHub Pages, so hosting is free, making it the only part of this project that respected a budget.",
    links: [
